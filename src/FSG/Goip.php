...
class Goip
{
	 ...
	 *        	The IP address of the GOIP
	 * @param int $port
	 *        	The port of the corresponding SIM card (ex. 9991)
	 * @param string $password
	 *        	The password set in GOIP management
	 */
	...
	protected function submitNumberRequest(MessageVO $message)
	{
		...
		$msg_len = strlen($msg_id);

		$msg = "SEND " . $msg_id . " 1 " . $message->getReceiver() . "";
		...
		for ($i = 1; $i <= 30; $i++)
		{
			...
			// echo "rcv $buffer\n";
			if (substr($buffer, 0, (3 + $msg_len)) === "OK " . $msg_id)
			{
				return true;
			}
			else 
				if (substr($buffer, 0, (5 + $msg_len)) === "WAIT " . $msg_id)
				{
					$retry = false;
				}
				else 
					if (substr($buffer, 0, (6 + $msg_len)) === "ERROR " . $msg_id)
					{
	...
	private function listenToReply(MessageVO $message, $phase, $expect)
	{
		for ($i = 1; $i <= 30; $i++)
		{
			...
			if (substr($buffer, 0, (6 + strlen($message->getId()))) === "ERROR " . $message->getId())
			{
				...
				if (substr($buffer, 0, (1 + strlen($expect) + strlen($message->getId()))) === $expect . " " . $message->getId())
				{
					return true;
				}
		}
